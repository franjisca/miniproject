...
public class FacilityPublicAPI {
	...
	public FacilityPublic callFacilityPublicByXML() {
		...
		urlBuffer.append("&" + "radius=" + "100000");	//설정값이 높으면 데이터 확인이 오래걸림 (100000정도가 적당)
		...
		try {
			...
			int count = 1;
			FacilityPublic facilityPublic = new FacilityPublic();
			...
			for(int i=0; i<nList.getLength(); i++) {
				...
				if(node.getNodeType() == Node.ELEMENT_NODE) {
					...
					System.out.println(getItem("toiletCo", eElement));

					facilityPublic.settoiletNo(getItem("toiletCo", eElement));
					facilityPublic.setswrmNo(getItem("swrmCo", eElement));
					facilityPublic.setwtrpNo(getItem("wtrplCo", eElement));
					facilityPublic.setposblFcltyClNo(getItem("posblFcltyCl", eElement));

					int result = 0;
					String msg = null;
					result = facilityInfoController.insertPublic(facilityPublic);
					msg = result > 0 ? "성공" : "실패";
					displayMsg(msg);
				}
			}
