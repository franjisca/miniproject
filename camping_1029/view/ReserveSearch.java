...
public class ReserveSearch {
	...
	public String cmpSearch(int num) {
		...
		return publicPlace(str, num);
	}

	public String publicPlace(String str, int num) {
		// url 담는 부분
		...
		try {
			...
			System.out.print("예약하고싶은 캠핑장 번호를 선택해주세요 : ");
			...
			for (int i = 0; i < nList.getLength(); i++) {
				...
				if (node.getNodeType() == Node.ELEMENT_NODE) {
					...
					if(itemValidation("facltNm", eElement, campStr) == 1) {
						String campName = getItem("facltNm", eElement);
						return campName;
